...
class KLineFloatingView : LinearLayout, KLineChartFrameLayout.OnKLineListener {
    // Hide ampl for TR and Toko, but show Ampl for cloudBase site, like Thailand, SA, etc.
    var showAmpl: Boolean = false
    ...
    private var floatingTvTxn: TextView? = null
    private var amplLayout: LinearLayout? = null
    private var txnLayout: LinearLayout? = null
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        val v = LayoutInflater.from(context).inflate(R.layout.view_kline_floating, this)
        ...
        floatingTvTxn = v.findViewById(R.id.floatingTvTxn)
        amplLayout = v.findViewById(R.id.ampl_layout)
        txnLayout = v.findViewById(R.id.txn_layout)
    ...
    override fun onCursorVisible(
        ...
        if (mKLineChartFrameLayout.onlyShowFloatingView && isCursorVisible) {
    ...
    override fun onKLineListener(
        ...
        if (showAmpl) {
            ...
            floatingTvVol?.text = StockUtils.getInstance()
                .parse2USString(item.vol, false, false, 2)
            floatingTvTxn?.text = StockUtils.getInstance()
                .parse2USString(item.cje, false, false, 2)
            floatingTvAmpl?.text = StockUtils.getInstance().getAmpl(item)
        } else {
            amplLayout?.gone()
            txnLayout?.gone()
        ...
        if (parent === mKLineChartFrameLayout) {
            if (cursorX > mKLineChartFrameLayout.width / 2.0f) {
